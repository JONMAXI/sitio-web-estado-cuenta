        ...
        <div class="card">
            ...
            <form method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nombre" name="nombre" value="{{ request.form.nombre or '' }}" placeholder="Escribe el nombre del cliente">
                </div>

                <!-- Campo ID Crédito / Dropdown dinámico -->
                {% if resultados %}
                <div class="mb-3">
                    <label class="form-label">Selecciona el crédito</label>
                    <select class="form-control" name="idCredito" required>
                        {% for r in resultados %}
                        <option value="{{ r.id_credito }}">
                            {{ r.Nombre_cliente }} - ID: {{ r.id_credito }} - Cliente: {{ r.id_cliente }} - Inicio: {{ r.Fecha_inicio }}
                        </option>
                        {% endfor %}
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="idCredito" name="idCredito" 
                        min="0" max="999999999" value="{{ request.form.idCredito or '' }}"
                        oninput="this.value = this.value.replace(/[^0-9]/g, '')">
                </div>
                {% endif %}

                <!-- Fecha de Corte Oculta -->
                <div class="mb-3" style="display: none;">
                    <label for="fechaCorte" class="form-label">Fecha de Corte</label>
                    <input type="date" class="form-control" id="fechaCorte" name="fechaCorte" required value="{{ fecha_actual_iso }}">
                </div>
...
<script>
    ...
    document.getElementById('fechaCorte').value = formattedToday;
</script>
</body>
</html>
